<script>
    ...
    function load_data() {
        ...
        table = $('#load-content').DataTable({
            ...
            ajax: {
                url: "<?= site_url('master/bendahara_pengeluaran/load'); ?>",
                type: 'POST'
            },
            columnDefs: [{
    ...
    $(document).on("click", "#tombol-tambah", function() {
        ...
        $.ajax({
            url: '<?= site_url('master/bendahara_pengeluaran/form'); ?>',
            type: "POST",
            data: {
                opsi: "add"
            },
    ...
    $(document).on("submit", "#form-tambah", function(e) {
        ...
        $.ajax({
            url: "<?= site_url('master/bendahara_pengeluaran/add'); ?>",
            type: "POST",
    ...
    $(document).on("click", "#tombol-new", function() {
        var self = this;
        $(self).attr('disabled', true);
        $(self).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i>");
        $.ajax({
            ...
            success: function(data) {
                $('#load-form-action').html(data);
                $(self).html('<i class="fa fa-user"></i> NEW TTD');
                $(self).attr('disabled', false);
            }
        });
    });
    ...
    $(document).on("submit", "#form-new", function(e) {
        e.preventDefault(e);
        var self = "#btn-new";
        var form_id = "#form-new";
        $(self).attr('disabled', true);
        $(self).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('master/bendahara_pengeluaran/new_ttd'); ?>",
            type: "POST",
            enctype: 'multipart/form-data',
            data: new FormData($(form_id)[0]),
            dataType: "json",
    ...
    $(document).on("click", "#tombol-ubah", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#load-form-action').html(data);
                $(self).html('<i class="fa fa-edit"></i> UBAH');
                $(self).attr('disabled', false);
            }
        });
    });
    ...
    $(document).on("submit", "#form-ubah", function(e) {
        ...
        $(self).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('master/bendahara_pengeluaran/edit'); ?>",
            type: "POST",
            enctype: 'multipart/form-data',
            data: new FormData($(form_id)[0]),
            dataType: "json",
