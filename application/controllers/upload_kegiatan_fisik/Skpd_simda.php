...
class Skpd extends CI_Controller
{
    ...
    public function load()
    {
        ...
        if ($this->akses['role'] == "master" or $this->akses['role'] == "admin" or $this->akses['role'] == "provinsi") {
            ...
            foreach ($result as $r) {
                ...
                if($total_pendapatan==0){$persen_total_belanja=0;}
    ...
    {
        $id_skpd = decrypt_url($encrypt_id);
        $row_log_upload = $this->mquery->select_id('log_upload', ['id_skpd' => $id_skpd, 'tahun' => 2022, 'bulan' => $bulan, 'st_data' => 2]);
        $skpd = $this->mquery->select_id('data_skpd', ['id_skpd' => $id_skpd]);
        $data = [
            "menu_active" => "upload_data",
            "submenu_active" => "upload_lra_skpd",
        ...
        if (!$this->upload->do_upload('file_upload')) {
            ...
            if ($new_file != "" || $new_file != NULL) {
                ...
                else
                {
                    ...
                    $anggaran='';
                    $jenis=1;
                    $cek_papbd = $this->mquery->select_id('setting_anggaran', ['id_setting' => 2]);
                    $tgl_papbd=$cek_papbd['papbd'];
                    $tgl_now=date('Y-m-d');
                    ...
                    foreach ($sheet as $row) {
                        if ($numrow > 10) {
                            ...
                            if ($cek_D != 0) {
                                ...
                                if($cek_jml_data1==0)
                                {
                                    if($hsl_stanggaran==2){$this->db->insert('data_uraian_kegiatan_pemko', $array_alokasi_pemko);}
                                }
                                else
                                {
                                    if($hsl_stanggaran==2){$this->db->update('data_uraian_kegiatan_pemko', $update_alokasi_pemko, ['kode_rekening' => $kode_rekening, 'tahun' => $post['tahun'], 'st_anggaran' => $hsl_stanggaran]);}
                                }
                                ...
                                if($cek_jml_data2==0)
                                {
                                    if($hsl_stanggaran==2){$this->db->insert('data_uraian_kegiatan_skpd', $array_alokasi_skpd);}
                                }
                                else
                                {
                                    if($hsl_stanggaran==2){$this->db->update('data_uraian_kegiatan_skpd', $update_alokasi_skpd, ['id_skpd' => $id_skpd, 'kode_rekening' => $kode_rekening, 'tahun' => $post['tahun'], 'st_anggaran' => $hsl_stanggaran]);}
                                }
                            }
                        }
                        $numrow++;
