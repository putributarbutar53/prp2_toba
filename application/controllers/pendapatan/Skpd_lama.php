...
class Skpd extends CI_Controller
{
    ...
    public function load()
    {
        ...
        $result = $this->mquery->select_by('data_realisasi_detail_skpd', ['bulan' =>  $cek_max['bulan'], 'tahun'=>$tahun, 'kode_rekening' => 4], 'persen DESC');
        $data = [];
        $no = 0;
        ...
        foreach ($result as $r) {
            ...
            $nama_skpd = "<a href=" . base_url("pendapatan/skpd/detail/".$tahun."/" . $encrypt_id) . ">" . $skpd['nama_skpd'] . "</a>";
                
            if($skpd['acuan_pendapatan']==0)
            {
                $row_anggaran = $this->mquery->select_id('data_uraian_kegiatan_skpd', ['tahun' => $tahun, 'id_skpd' => $r['id_skpd'], 'kode_rekening' => 4, 'st_anggaran' => $st_anggaran]);
                $hsl_pendapatan=$row_anggaran['anggaran'];
            }
            else{$hsl_pendapatan=$skpd['pendapatan'];}
            ...
            if ($hsl_pendapatan != 0) {
                ...
                $row = [
                    'no' => $no,
                    'nama_skpd' => $nama_skpd,
                    'pagu' => "Rp" . number_format($hsl_pendapatan, 0, ',', '.'),
