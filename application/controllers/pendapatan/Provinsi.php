...
class Provinsi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $rst_realisasi_pendapatan = (float)($row_realisasi_pendapatan['realisasi']);

        $persen_pad = round(($struktur_anggaran_provinsi['pad'] / $struktur_anggaran_provinsi['pendapatan'] * 100), 2);
        $persen_transfer = round(($struktur_anggaran_provinsi['transfer'] / $struktur_anggaran_provinsi['pendapatan'] * 100), 2);
        $persen_pad_lain = round(($struktur_anggaran_provinsi['pad_lain'] / $struktur_anggaran_provinsi['pendapatan'] * 100), 2);
        $persen_realisasi = round(($rst_realisasi_pendapatan / $struktur_anggaran_provinsi['pendapatan'] * 100), 2);
        ...
        for ($i23 = 1; $i23 < $batas; $i23++) {
            ...
            if ($i23 == $cek_max['bulan']) {
                ...
                $persen_pad_terakhir = $pad_terakhir / $struktur_anggaran_provinsi['pad'] * 100;
                $persen_transfer_terakhir = $transfer_terakhir / $struktur_anggaran_provinsi['transfer'] * 100;
                $persen_lain2_terakhir = $lain2_terakhir / $struktur_anggaran_provinsi['pad_lain'] * 100;
            }
        }
        ...
        $this->load->view('pendapatan/provinsi/dashboard', $data);
    }
