...
class Bendahara_pengeluaran extends CI_Controller
{
    ...
    public function index()
    {
        $data = [
            ...
            "submenu_active" => "bendahara-pengeluaran"
        ];
        $this->load->view('master/bendahara_pengeluaran/view', $data);
    }
    ...
    public function load()
    {
        ...
        if ($this->akses['role'] == 'skpd') {
            ...
            $result = $this->master->get_ttd1($user['id_skpd']);
        } else {
            $result = $this->master->get_ttd1();
        }
        $data = [];
        $no = 0;
        foreach ($result as $r) {
    ...
    public function form()
    {
        ...
        if ($opsi == "add") {
            ...
            $this->load->view('master/bendahara_pengeluaran/form_add', $data);
        } elseif ($opsi == "new") {
            $id_skpd = htmlspecialchars($this->input->post('skpd', TRUE));
            $data = [
                "skpd" => $this->mquery->select_id('data_skpd', ['id_skpd' => $id_skpd])
            ];
            $this->load->view('master/bendahara_pengeluaran/form_new', $data);
        } elseif ($opsi == "edit") {
            $id = htmlspecialchars($this->input->post('id', TRUE));
            $data = [
                'bendahara_pengeluaran' => $this->master->get_ttd_id1($id)
            ];
            $this->load->view('master/bendahara_pengeluaran/form_edit', $data);
        } else {
    ...
    private function _rule_form()
    {
        ...
        $this->form_validation->set_rules('nip_ttd', 'NIP bendahara pengeluaran', 'required|trim');
        $this->form_validation->set_rules('nama_ttd', 'Nama bendahara pengeluaran', 'required|trim');
    ...
    private function _send_error($params)
    {
        if ($params == 'skpd') {
            $errors = [
                'nama_skpd' => "<p>Data bendahara pengeluaran Sudah Ada Di Skpd Ini</p>",
                'nip_ttd' => form_error('nip_ttd'),
                'nama_ttd' => form_error('nama_ttd'),
                'ttd' => form_error('ttd')
            ];
            $data = ['status' => FALSE, 'errors' => $errors, 'pesan' => 'Data bendahara pengeluaran Sudah Ada'];
    ...
    function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $cek_pa = $this->mquery->count_data('bendahara_pengeluaran', ['id_skpd' => $id_skpd]);
            if ($cek_pa > 0) {
                ...
                if (!$this->upload->do_upload('ttd')) {
                    ...
                    $string = ['bendahara_pengeluaran' => $array];
                    $log = simpan_log("new bendahara pengeluaran", json_encode($string));
                    $res = $this->mquery->insert_data('bendahara_pengeluaran', $array, $log);
    ...
    function new_ttd()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!$this->upload->do_upload('ttd')) {
                ...
                $cek_ttd = $this->mquery->count_data('bendahara_pengeluaran', ['id_skpd' => $id_skpd]);
                if ($cek_ttd > 0) {
                    $temp = $this->mquery->select_id('bendahara_pengeluaran', ['id_skpd' => $id_skpd, 'is_active' => 'Y']);
                    $array_old = [
                        'is_active' => 'N'
                    ];
                    $string = ['bendahara_pengeluaran' => ['old' => $temp, 'new' => $array]];
                    $log = simpan_log("new bendahara pengeluaran", json_encode($string));
                    $res = $this->master->insert_new_ttd1($array, $array_old, $temp['id_ttd'], $log);
                } else {
                    $string = ['bendahara_pengeluaran' => $array];
                    $log = simpan_log("new bendahara pengeluaran", json_encode($string));
                    $res = $this->mquery->insert_data('bendahara_pengeluaran', $array, $log);
    ...
    function edit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($_FILES['ttd']['name'])) {
                ...
                $temp = $this->mquery->select_id('bendahara_pengeluaran', ['id_ttd' => $id_ttd]);
                $string = ['bendahara_pengeluaran' => ['old' => $temp, 'new' => $array]];
                $log = simpan_log("update bendahara pengeluaran", json_encode($string));
                $res = $this->mquery->update_data('bendahara_pengeluaran', $array, ['id_ttd' => $id_ttd], $log);
                $data = ['status' => TRUE, 'notif' => $res];
                $this->output->set_content_type('application/json')->set_output(json_encode($data));
            } else {
                if (!$this->upload->do_upload('ttd')) {
